...
import { ModuleHeader, FeedbackBanner, StatCard, PageLayout, ActionButton } from '@/components/modules/ModuleHeader';
...
export function CommunicationModule() {
  ...
  const loadPrayers = React.useCallback(async () => {
    try {
      const prayerRes = await apiRequest<unknown>('care/prayer', { method: 'GET' });
      setPrayers(parseApiResponse<PrayerRow[]>(prayerRes) ?? []);
    } catch {
      /* Prayer tab optional — 403 for comms-only roles must not block the hub */
      setPrayers([]);
    }
  }, []);
  ...
  const load = React.useCallback(async () => {
    try {
      setError(null);
      setLoading(true);
      const [hubRes, logRes] = await Promise.all([
  ...
  return (
    <PageLayout>
      <ModuleHeader
        ...
        actions={
          <>
            <ActionButton label="Refresh" icon={RefreshCw} variant="secondary" onClick={() => void load()} />
            <ActionButton label="Compose campaign" icon={Send} variant="primary" onClick={() => setTab('compose')} />
          </>
        }
      />
      ...
      {tab === 'command' && hub && (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          <StatCard label="30d deliveries" value={hub.analytics.deliveries30d} icon={Mail} />
          <StatCard label="Open rate" value={`${hub.analytics.openRate}%`} icon={BarChart3} iconColor="text-indigo-600" iconBg="bg-indigo-50" />
          <StatCard label="Engagement" value={`${hub.analytics.engagementRate}%`} icon={Heart} iconColor="text-emerald-600" iconBg="bg-emerald-50" />
          <StatCard label="In-app (7d)" value={hub.analytics.notifications7d} icon={Bell} />
