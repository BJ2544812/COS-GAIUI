    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Card className="rounded-3xl border-rose-100">
        <CardHeader>
          ...
          <CardDescription>Pastoral recommendations from attendance and guest data</CardDescription>
        </CardHeader>
        ...
        <CardContent className="space-y-2">
          {data.ministryStress.slice(0, 8).map((v) => (
            <div key={v.memberId} className="flex justify-between p-3 rounded-xl bg-slate-50 text-sm">
              ...
              <span className="text-[10px] font-black uppercase text-amber-700">{v.activeAssignments} roles</span>
            </div>
          ))}
          <button
            type="button"
